...
func _ready():
	get_tree().create_tween().tween_property($Sprite2D, "modulate:a", 0, DURATION)
	get_tree().create_timer(DURATION).timeout.connect(self.queue_free)
...
func _try_level_up():
	for area in get_overlapping_areas():
		if area.get_parent() is TileObject:
			...
			tile_obj.try_level_up_from_snail_trail(0.05)
